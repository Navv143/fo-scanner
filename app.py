...
# --- 1. CONFIG & UI ---
st.set_page_config(page_title="PRO-QUANT ELITE v14.1", layout="wide", initial_sidebar_state="expanded")
...
def load_ui():
    try:
        ...
        st.info("Applying default dark theme...")

load_ui()
st_autorefresh(interval=3 * 60 * 1000, key="global_sync")

# --- 2. COMPLETE 212+ STOCK & SECTOR DATABASE ---
SECTOR_MAP = {
    ...
    "PHARMA": ["SUNPHARMA.NS", "CIPLA.NS", "DRREDDY.NS", "DIVISLAB.NS", "AUROPHARMA.NS", "LUPIN.NS", "ALKEM.NS", "BIOCON.NS", "GLENMARK.NS", "ZYDUSLIFE.NS", "APOLLOHOSP.NS"],
    ...
    "OTHERS": ["SRF.NS", "UPL.NS", "DEEPAKNTR.NS", "POLYCAB.NS", "DIXON.NS", "INDIGO.NS", "IEX.NS", "PVRINOX.NS", "ZEEL.NS", "IDEA.NS", "MCX.NS"]
}
...
def fetch_master_data():
    ...
    for t in all_tkr:
        try:
            ...
            if t in ALL_STOCKS:
                score = get_hybrid_score({'LTP': p, 'Chg%': chg_p, 'ROC': roc}, prev['High'], prev['Low'], adr, obv_t)
                sig = "🚀 BULLISH" if p > prev['High'] else "📉 BEARISH" if p < prev['Low'] else "Neutral"
                sec = next((k for k, v in SECTOR_MAP.items() if t in v), "Other")
                s_rows.append({"Sector": sec, "Symbol": t.replace(".NS",""), "LTP": round(p, 2), "Chg%": round(chg_p, 2), "ROC": round(roc, 2), "OBV": obv_t, "Score": score, "Signal": sig, "Vol_Ratio": round(curr['Volume']/prev['Volume'], 2)})
...
with st.sidebar:
    ...
    menu = option_menu(None, ["Terminal", "Impulse Radar", "Sector Analysis", "Watchlist"], icons=["speedometer", "lightning", "grid", "list"], default_index=0)
    st.info(f"Sync: {datetime.now(pytz.timezone('Asia/Kolkata')).strftime('%H:%M:%S')} IST")

# --- 5. CRASH-PROOF DASHBOARD ---
if menu == "Terminal":
    if not df_i.empty:
        ...
        for row_start in range(0, 6, 3):
            ...
            for j in range(3):
                ...
                # CRASH FIX: Check if the index exists in the data
    ...
    if not df_s.empty:
        with c1:
            for _, r in df_s[(df_s['Signal']=="🚀 BULLISH") & (df_s['Score'] >= 60)].nlargest(3, 'Score').iterrows():
                st.markdown(f"<div class='mobile-card' style='border-color:#10b981'><b>{r['Symbol']}</b> | Score: {r['Score']} | <span style='color:#10b981'>{r['Chg%']:+.2f}%</span></div>", unsafe_allow_html=True)
        with c2:
            for _, r in df_s[(df_s['Signal']=="📉 BEARISH") & (df_s['Score'] >= 60)].nlargest(3, 'Score').iterrows():
                st.markdown(f"<div class='mobile-card' style='border-color:#f43f5e'><b>{r['Symbol']}</b> | Score: {r['Score']} | <span style='color:#f43f5e'>{r['Chg%']:+.2f}%</span></div>", unsafe_allow_html=True)
